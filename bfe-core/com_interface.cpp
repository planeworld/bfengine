...
void CComInterface::callWriters(const std::string& _strQueue)
{
    ...
    auto it = m_WriterQueues.find(_strQueue);
    if (it == m_WriterQueues.end())
    {
        WARNING_MSG("Com Interface", "Writer queue <" << _strQueue << "> doesn't exist. Skipping execution.")
    }
